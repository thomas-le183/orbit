...
import { Card, CardContent, CardFooter } from "@orbit/ui/components/card";
import {
	Field,
	FieldDescription,
	FieldError,
		...
		return (
			<Card className={cn(className)} {...props}>
				<CardContent className="flex flex-col items-center gap-4 text-center">
					...
					>
						{sendVerification.isPending ? "Sending…" : "Resend email"}
					</Button>
					<FieldDescription>
						Wrong email?{" "}
						<button
					...
					<FieldGroup>
						...
						<Field className="flex flex-col gap-3">
							<Button
								variant="outline"
								type="button"
								className="w-full border-2 py-5 text-sm font-medium shadow-sm hover:shadow-md transition-shadow"
								disabled={googlePending}
								onClick={async () => {
									setGooglePending(true);
									await authClient.signIn.social({ provider: "google", callbackURL: `${import.meta.env.VITE_WEB_BASE_URL}/` });
								}}
							>
								<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="size-5 shrink-0">
